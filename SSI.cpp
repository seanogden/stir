...
void SSI::insertSigma(TerminatorInst *TI, Instruction *I) {
  ...
  errs() << TI->getNumSuccessors() << " successors\n";
  for (unsigned i = 0, e = TI->getNumSuccessors(); i < e; ++i) {
    // Next Basic Block
    BasicBlock *BB_next = TI->getSuccessor(i);

    if (BB_next == BB) errs() << "BB_next == BB\n";
    if (BB_next->getSinglePredecessor() == NULL) errs() << "BB_next->getSinglePredecessor() == NULL\n";
    if (!dominateAny(BB_next, I)) errs() << "!dominateAny(BB_next, I)\n";

    if (BB_next != BB &&
        BB_next->getSinglePredecessor() != NULL &&
        dominateAny(BB_next, I)) {
      errs() << "Inserting SSI_SIG\n";
      PHINode *PN = PHINode::Create(I->getType(), 0, SSI_SIG, BB_next->begin());
      PN->addIncoming(I, BB);
      sigmas[PN] = I;
      errs() << "Insert SIGMA\n";
      created.insert(PN);
      ...
      for (std::set<BasicBlock *>::iterator DF_BB_begin =
        ...
        if (BB_visited.insert(BB_dominated) &&
          ...
          PHINode *PN = PHINode::Create(
              (*I)->getType(), 0, SSI_PHI, BB_dominated->begin());
...
void SSI::rename(BasicBlock *BB) {
  ...
  // Then store b in the value_stack as the new definition of a.
